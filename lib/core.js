...
core.Reverse.prototype.run = function (parentResults) {
    ...
    // sadly the parentResults are useless to us, as those are the results for
    // <subject> <predicate> ?  and we're trying to get ? <predicate> <subject> .

    // FIXME: refactor things so that we don't query for data in Query.prototype.query and
    // then throw it away here.

    var id = parentResults['@id'][0].subject;
    var predicate = self.parent.namespaces.expandTerm (this.predicate);

    return self.parent.connection.query ({
            subject: null,
            predicate: predicate,
            object: id
    }).then (function (results) {
        return when.map (results, function (item) {
...
core.buildModel = function (queryResult, model) {
    var resolve = function (value) {
        ...
        } else if (value instanceof core.Reverse) {
            return value.run (queryResult);
        } else {
